...
export function renderSettingsPanelContent(panel: HTMLElement): void {
  ...
  mcpGroup.className = "mo-settings-group";

  const mcpLabel = document.createElement("span");
  mcpLabel.className = "mo-settings-label";
  mcpLabel.textContent = "MCP Server";
  mcpGroup.appendChild(mcpLabel);

  const mcpRow = document.createElement("label");
  mcpRow.className = "mo-settings-toggle";
  const mcpToggleLabel = document.createElement("span");
  mcpToggleLabel.textContent = "Enable MCP bridge";
  const mcpCheck = document.createElement("input");
  mcpCheck.type = "checkbox";
  mcpCheck.checked = state.settings.mcpEnabled;
  mcpCheck.addEventListener("change", () => {
    state.settings.mcpEnabled = mcpCheck.checked;
    saveSettings();
    renderToolbarPanel();
  });
  mcpRow.append(mcpToggleLabel, mcpCheck);
  mcpGroup.appendChild(mcpRow);

  if (state.settings.mcpEnabled) {
    const quickStart = document.createElement("p");
    quickStart.className = "mo-mcp-quickstart";
    quickStart.textContent = "Default local setup: start the Missouri MCP server once, then turn on Enable MCP bridge. Missouri reconnects automatically when the server or page appears. No browser restart and no shared token are required for normal local use.";
    mcpGroup.appendChild(quickStart);

    const portRow = document.createElement("div");
    portRow.className = "mo-mcp-detail-row";
    const portLabel = document.createElement("span");
    portLabel.className = "mo-mcp-detail-label";
    portLabel.textContent = "Port";
    const portInput = document.createElement("input");
    portInput.type = "number";
    portInput.className = "mo-settings-port-input";
    portInput.value = String(state.settings.mcpPort);
    portInput.min = "1024";
    portInput.max = "65535";
    portInput.addEventListener("change", () => {
      const value = parseInt(portInput.value, 10);
      if (value >= 1024 && value <= 65535) {
        state.settings.mcpPort = value;
        saveSettings();
        renderToolbarPanel();
      } else {
        portInput.value = String(state.settings.mcpPort);
    ...
    mcpGroup.appendChild(portRow);

    const statusRow = document.createElement("div");
    statusRow.className = "mo-mcp-detail-row";
    const statusLabel = document.createElement("span");
    statusLabel.className = "mo-mcp-detail-label";
    statusLabel.textContent = "Status";
    const statusValue = document.createElement("span");
    statusValue.className = "mo-mcp-detail-value";
    const statusDot = document.createElement("span");
    statusDot.className = "mo-mcp-status-dot";
    const statusText = document.createElement("span");
    if (state.mcpConnected === true) {
      statusDot.dataset.status = "connected";
      statusText.textContent = "Connected";
    } else if (state.mcpConnected === false) {
      statusDot.dataset.status = "disconnected";
      statusText.textContent = "Waiting for local server";
    } else {
      statusText.textContent = "Checking…";
    }
    statusValue.append(statusDot, statusText);
    statusRow.append(statusLabel, statusValue);
    mcpGroup.appendChild(statusRow);
    ...
    chrome.runtime.sendMessage({ type: "GET_MCP_STATUS" }, (response) => {
      ...
      statusText.textContent = connected ? "Connected" : "Waiting for local server";
    });

    mcpGroup.appendChild(renderMcpSetupInstructions());
  }

  panel.appendChild(mcpGroup);
...
export function renderMcpSetupInstructions(): HTMLElement {
  const wrap = document.createElement("div");
  wrap.className = "mo-mcp-instructions";

  const toggle = document.createElement("button");
  toggle.className = "mo-mcp-instructions-toggle";
  toggle.textContent = "Setup instructions";
  const arrow = document.createElement("span");
  arrow.className = "mo-mcp-instructions-arrow";
  arrow.textContent = "\u25B8";
  toggle.prepend(arrow);

  const body = document.createElement("div");
  body.className = "mo-mcp-instructions-body";
  ...
  const port = state.settings.mcpPort;

  const ccTitle = document.createElement("strong");
  ccTitle.className = "mo-mcp-instr-heading";
  ccTitle.textContent = "Claude Code";
  body.appendChild(ccTitle);

  const ccDesc = document.createElement("p");
  ccDesc.className = "mo-mcp-instr-text";
  ccDesc.textContent = "Run this once in your terminal. Claude talks to Missouri over stdio, and Missouri opens the local WebSocket bridge itself:";
  body.appendChild(ccDesc);

  const ccCode = document.createElement("pre");
  ccCode.className = "mo-mcp-instr-code";
  ccCode.textContent = `claude mcp add missouri -- \\\n  node /path/to/missouri-mcp-server/dist/index.js`;
  body.appendChild(ccCode);

  const ccAlt = document.createElement("p");
  ccAlt.className = "mo-mcp-instr-text";
  ccAlt.textContent = "Or add to .claude/settings.json under mcpServers:";
  body.appendChild(ccAlt);

  const ccJson = document.createElement("pre");
  ccJson.className = "mo-mcp-instr-code";
  ccJson.textContent = `"missouri": {\n  "command": "node",\n  "args": ["path/to/missouri-mcp-server/dist/index.js"],\n  "env": {\n    "MISSOURI_WS_PORT": "${port}"\n  }\n}`;
  body.appendChild(ccJson);

  const curTitle = document.createElement("strong");
  curTitle.className = "mo-mcp-instr-heading";
  curTitle.textContent = "Cursor / VS Code (Copilot)";
  body.appendChild(curTitle);

  const curDesc = document.createElement("p");
  curDesc.className = "mo-mcp-instr-text";
  curDesc.textContent = "Add to .cursor/mcp.json or VS Code MCP config:";
  body.appendChild(curDesc);

  const curJson = document.createElement("pre");
  curJson.className = "mo-mcp-instr-code";
  curJson.textContent = `{\n  "mcpServers": {\n    "missouri": {\n      "command": "node",\n      "args": ["path/to/missouri-mcp-server/dist/index.js"],\n      "env": {\n        "MISSOURI_WS_PORT": "${port}"\n      }\n    }\n  }\n}`;
  body.appendChild(curJson);

  const genTitle = document.createElement("strong");
  genTitle.className = "mo-mcp-instr-heading";
  genTitle.textContent = "Other agents";
  body.appendChild(genTitle);

  const genDesc = document.createElement("p");
  genDesc.className = "mo-mcp-instr-text";
  genDesc.textContent = "Any MCP-compatible agent can connect via stdio. The server binary is:";
  body.appendChild(genDesc);

  const genCode = document.createElement("pre");
  genCode.className = "mo-mcp-instr-code";
  genCode.textContent = `node path/to/missouri-mcp-server/dist/index.js`;
  body.appendChild(genCode);

  const envNote = document.createElement("p");
  envNote.className = "mo-mcp-instr-text mo-mcp-instr-note";
  envNote.textContent = `Set MISSOURI_WS_PORT=${port} if you changed the port from the default (18462). You only need MISSOURI_SECRET if you explicitly want a shared localhost auth token; the normal local setup does not require it.`;
  body.appendChild(envNote);

  const noRestartNote = document.createElement("p");
  noRestartNote.className = "mo-mcp-instr-text mo-mcp-instr-note";
  noRestartNote.textContent = "No restart path: start the server, open the page with Missouri enabled, and the extension will reconnect automatically within a few seconds.";
  body.appendChild(noRestartNote);

  const skillTitle = document.createElement("strong");
  skillTitle.className = "mo-mcp-instr-heading";
  skillTitle.textContent = "Skill (recommended)";
  body.appendChild(skillTitle);

  const skillDesc = document.createElement("p");
  skillDesc.className = "mo-mcp-instr-text";
  skillDesc.textContent = "Install the Missouri skill from your project root so your coding agent understands Missouri annotations:";
  body.appendChild(skillDesc);

  const skillCode = document.createElement("pre");
  skillCode.className = "mo-mcp-instr-code";
  skillCode.textContent = SKILL_INSTALL_COMMAND;
  body.appendChild(skillCode);

  const skillNote = document.createElement("p");
  skillNote.className = "mo-mcp-instr-text mo-mcp-instr-note";
  skillNote.textContent = "npx skills auto-detects supported harnesses and installs the right hidden provider folder. Cursor and Gemini still need their agent-skill features enabled in the tool itself.";
  body.appendChild(skillNote);

  const skillDocs = document.createElement("div");
  skillDocs.className = "mo-mcp-doc-links";
  for (const target of SKILL_INSTALL_TARGETS) {
    if (!target.docsUrl) continue;
    const link = document.createElement("a");
    link.className = "mo-info-link mo-mcp-doc-link";
    link.href = target.docsUrl;
    link.target = "_blank";
    link.rel = "noopener";
    link.textContent = `${target.label} docs`;
    skillDocs.appendChild(link);
  }
  body.appendChild(skillDocs);

  const copyBtn = document.createElement("button");
  copyBtn.className = "mo-mcp-copy-btn";
  ...
  copyBtn.addEventListener("click", (e) => {
    ...
    const config = JSON.stringify({
      missouri: {
        command: "node",
        args: ["path/to/missouri-mcp-server/dist/index.js"],
        env: { MISSOURI_WS_PORT: String(port) }
      }
    }, null, 2);
